...
Duas colunas 0/1 para três espécies. `Adelie` é o nível de referência:
é a linha $(1, 0, 0)$.

::: {.nota}
$k$ grupos exigem $k-1$ colunas. Se houvesse uma terceira coluna para
`Adelie`, a matriz seria singular — o intercepto já a contém.
:::

---

## Coeficientes são diferenças

```{r}
coef(m)
pinguins |> group_by(species) |> summarise(media = mean(body_mass_g))
```

`Gentoo` pesa em média `r round(coef(m)[3])` g a mais que `Adelie`.
...
- Gelman, A., Hill, J. & Vehtari, A. 2020. *Regression and Other
  Stories*. Cambridge University Press. Caps. 10–12.
- Lenth, R. `emmeans`: estimated marginal means.
  <https://cran.r-project.org/package=emmeans>
- Quinn, G.P. & Keough, M.J. 2023. *Experimental Design and Data
  Analysis for Biologists*. 2ª ed. Caps. 8–9.
